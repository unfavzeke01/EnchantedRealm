...
export function AdminManagement() {
  ...
  const createAdminMutation = useMutation({
    mutationFn: async (data: { username: string; password: string; nickname: string; role: string }) => {
  ...
  const toggleAdminStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Admin account status has been updated successfully.",
      });
    },
    onError: (error) => {
      toast({
  ...
  const handleCreateAdmin = (e: React.FormEvent) => {
    ...
    createAdminMutation.mutate({ username, password, nickname, role });
  };
    ...
    <Card>
      ...
      <CardContent>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8">
            <p className="text-gray-600">No admin accounts found. Create the first admin account above.</p>
          </div>
        ) : (
